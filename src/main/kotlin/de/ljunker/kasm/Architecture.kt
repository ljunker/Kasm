...
object Architecture {
    ...
    const val ADDRESS_REGISTER_COUNT = 2

    const val WORD_BITS = 8
    const val WORD_MASK = 0xFF
    const val WORD_SIGN_BIT = 0x80
    const val WORD_VALUE_COUNT = WORD_MASK + 1

    const val ADDRESS_BITS = 16
    const val ADDRESS_MASK = 0xFFFF
    const val ADDRESS_SPACE_SIZE = ADDRESS_MASK + 1
    const val MEMORY_SIZE = ADDRESS_SPACE_SIZE

    val wordRange: IntRange = 0..WORD_MASK
    val addressRange: IntRange = 0..ADDRESS_MASK

    fun normalizeWord(value: Int): Int =
        value and WORD_MASK

    fun normalizeAddress(value: Int): Int =
        value and ADDRESS_MASK
